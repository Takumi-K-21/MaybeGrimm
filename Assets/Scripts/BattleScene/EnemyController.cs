...
public class EnemyController : MonoBehaviour {
    ...
    // •Ï”‚Ì‰Šú‰»
    void Start() {
        //BattleScene.GameManager.isGameResume = true;
    ...
    void Attack() {
        if (rand == 0) { // ”j‰ó‰Â”\‚È’e‚ğ”­Ë
            ...
            GameObject bullet = Instantiate(bullet_01, vec1, Quaternion.Euler(0f, 0f, 90f)) as GameObject;
            EnemyBullet bulletScript = bullet.GetComponent<EnemyBullet>();
            bulletScript.setVelocity(v_x * direction_x, v_y * direction_y, v_z * direction_z);

            intervalTime = Random.Range(0f, 1f); // 0‚©‚ç3‚Ì”ÍˆÍ‚Å—”‚ğ¶¬

            Debug.Log("bullet!");
        } else if (rand == 1) { // ”j‰ó•s‰Â”\‚È’e‚ğ”­Ë
            ...
            EnemyBullet3 bulletScript = bullet.GetComponent<EnemyBullet3>();
            bulletScript.setVelocity(v_x * direction_x, v_y * direction_y, v_z * direction_z);

            intervalTime = Random.Range(0f, 1f); // 0‚©‚ç3‚Ì”ÍˆÍ‚Å—”‚ğ¶¬

            Debug.Log("bullet!");
        } else if (rand == 2 && !jumpNow) { // ƒWƒƒƒ“ƒvƒAƒNƒVƒ‡ƒ“
        ...
        if (jumpNow == true) {
            if (other.gameObject.CompareTag("Ground")) {
                ...
                if (r == 0) {
                    ...
                    for (int i = 0; i < n; i++) {
                        GameObject bullet = Instantiate(wave_01, vec, transform.rotation) as GameObject;
